...
export const REWAQ_BILLING_CURRENCY = "USD" as const;
...
export const REWAQ_PLANS: Record<RewaqPlanCode, RewaqPlanDefinition> = {
  ...
  scale: {
    ...
    monthlyPriceUsd: 350,
    description: "للمطعم الكبير والمتعدد الأقسام مع المحاسبة المتقدمة والخدمات المالية والأتمتة.",
...
export function planHasModule(planCode: unknown, module: RewaqModule) {
  return isRewaqPlanCode(planCode) && REWAQ_PLANS[planCode].modules.includes(module);
}
...
export const MODULE_ROUTE_RULES: readonly ModuleRouteRule[] = [
  ...
  { prefix: "/dashboard/branches", module: "administration" },
  { prefix: "/dashboard/settings", module: "administration" },
  { prefix: "/dashboard/billing", module: "administration" },
  ...
  { prefix: "/dashboard", module: "dashboard", exact: true },
] as const;

export function moduleForPath(pathname: string): RewaqModule | null {
  const path = pathname.split(/[?#]/, 1)[0].replace(/\/$/, "") || "/";
  return (
    MODULE_ROUTE_RULES.find((rule) =>
      rule.exact ? path === rule.prefix : path === rule.prefix || path.startsWith(`${rule.prefix}/`),
    )?.module ?? null
  );
}
