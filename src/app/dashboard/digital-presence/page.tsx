        ...
        <Card className="xl:sticky xl:top-20">
          ...
          <CardContent>
            <ActionForm action={saveRestaurantSiteAction} submitLabel={site ? "حفظ إعدادات الموقع" : "إنشاء الموقع كمسودة"} className="space-y-4">
              ...
              <div className="grid gap-2"><Label htmlFor="branchId">القسم المرتبط</Label><Select id="branchId" name="branchId" defaultValue={site?.branchId ?? ""}><option value="">كل الأقسام</option>{branches.map((branch) => <option key={branch.id} value={branch.id}>{branch.name}</option>)}</Select></div>
              ...
              menuItems.map((item) => {
                ...
                return (
                  <ActionForm key={item.id} action={setRestaurantSiteMenuItemAction} submitLabel="حفظ ربط الصنف" className="rounded-xl border p-4">
                    ...
                    {!canPublish && <p className="mb-3 rounded-lg bg-amber-50 p-2 text-xs text-amber-800">الصنف متوقف أو يتبع قسمًا مختلفاً، لذلك لا يمكن إظهاره.</p>}
