...
create type social_platform as enum ('facebook', 'instagram', 'telegram', 'tiktok', 'x', 'google_business');
...
create table menu_item_recipe_mapping (
  id uuid primary key default gen_random_uuid(),
  organization_id uuid not null references organizations(id) on delete cascade,
...
create index branch_stock_org_branch_idx on branch_stock (organization_id, branch_id);
create index stock_movements_org_branch_item_idx on stock_movements (organization_id, branch_id, item_id, created_at desc);
...
create index purchase_orders_org_branch_idx on purchase_orders (organization_id, branch_id, status);
create index customer_invoices_org_branch_idx on customer_invoices (organization_id, branch_id, issued_at desc);
create index recipes_org_idx on recipes (organization_id);
create index menu_items_org_idx on menu_items (organization_id, branch_id);
create index social_posts_org_status_idx on social_posts (organization_id, status, scheduled_at);
create index notifications_org_user_idx on notifications (organization_id, user_id, read_at);
