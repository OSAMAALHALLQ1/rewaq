-- Migration: 010_fix_department_api_keys
-- Description: Drops legacy department tables and creates correct schema for
-- department_api_keys and internal_messages (replaces supabase/migrations/006)

-- 1. Drop legacy tables from old supabase migration 006
-- Order matters: drop tables with FK references first
drop table if exists public.department_api_keys cascade;
drop table if exists public.department_members cascade;
drop table if exists public.departments cascade;

-- 2. Create department_api_keys Table
...
-- 6. RLS Policies for department_api_keys
...
-- 7. RLS Policies for internal_messages
...
-- Staff can view messages sent to their role, sent by them, or general group (NULL)
