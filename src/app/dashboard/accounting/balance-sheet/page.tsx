...
import { Scale, ShieldAlert, ArrowLeft, RefreshCw, Layers, TrendingUp } from "lucide-react";
import { PageHeader } from "@/components/page-header";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@/components/ui/table";
import { Badge } from "@/components/ui/badge";
import { getBalanceSheetData } from "@/server/queries/accounting";
...
export default async function BalanceSheetPage() {
  ...
  return (
    <>
      ...
      <PageHeader
        title="تقرير الميزانية العمومية (Balance Sheet)"
        description="تقرير يعرض أصول المنشأة، التزاماتها، وحقوق الملكية للمساهمين في تاريخ محدد."
      />
        ...
        <Card className={`backdrop-blur-md border shadow-md rounded-2xl overflow-hidden ${data.balanced ? "bg-emerald-50/50 border-emerald-200" : "bg-red-50/50 border-red-200"}`}>
          <CardContent className="p-5 flex items-center justify-between flex-wrap gap-4 text-right">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-sm font-bold text-slate-900">توازن ميزانية الفترة الحالية</h2>
                <p className="text-xs text-slate-500 mt-0.5">
                  {data.balanced 
                    ? "الميزانية متزنة بالكامل. يتساوى إجمالي الأصول مع إجمالي الالتزامات وحقوق الملكية." 
                    : "يوجد عدم اتزان في القيود المحاسبية. يرجى مراجعة توازن دفتر اليومية العام."}
                </p>
              </div>
            </div>
            <Badge tone={data.balanced ? "success" : "danger"} className="h-9 px-4 text-xs font-bold rounded-xl border shadow-sm flex items-center">
              {data.balanced ? "متزنة بالكامل" : "تحتاج تدقيق"}
            </Badge>
          </CardContent>
        </Card>
      </div>
              ...
              <Table className="text-xs">
                ...
                <TableBody>
                  {data.assets.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={2} className="text-center py-6 text-slate-450">لا توجد أرصدة أصول حالياً</TableCell>
                    </TableRow>
                  ) : (
                    data.assets.map((item) => (
                      <TableRow key={item.code} className="hover:bg-slate-50/30 transition-colors">
                        <TableCell className="py-2.5 px-5 text-slate-650">{item.code} - {item.name}</TableCell>
                        <TableCell className="text-left py-2.5 px-5 font-mono text-slate-800">{formatCurrency(item.balance)}</TableCell>
                      </TableRow>
                    ))
                  )}
                </TableBody>
              ...
              <Table className="text-xs">
                ...
                <TableBody>
                  ...
                  <TableRow className="bg-slate-50/40 font-bold">
                    <TableCell colSpan={2} className="py-2 px-5 text-slate-800">1. الالتزامات (Liabilities)</TableCell>
                  </TableRow>
                  ...
                  <TableRow className="font-bold border-b text-slate-750">
                    <TableCell className="py-2 px-5 pl-8">مجموع الالتزامات</TableCell>
                    <TableCell className="text-left py-2 px-5 font-mono">{formatCurrency(data.liabilitiesTotal)}</TableCell>
                  </TableRow>

                  {/* Equity Subsection */}
                  <TableRow className="bg-slate-50/40 font-bold border-t">
                    <TableCell colSpan={2} className="py-2 px-5 text-slate-800">2. حقوق الملكية (Equity)</TableCell>
                  </TableRow>
                  ...
                  {/* Retained Earnings (Profit of current period) */}
                  <TableRow className="hover:bg-slate-50/30 transition-colors">
                    <TableCell className="py-2.5 px-5 pl-8 text-slate-600">3999 - الأرباح المحتجزة للفترة الحالية</TableCell>
